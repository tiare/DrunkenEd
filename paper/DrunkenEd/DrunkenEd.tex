...
\begin{abstract}
\drunkened\ is a 2D balance game that was specifically designed for public displays. We show that this casual game is well suited for public context and that camera based body tracking offers convenient interaction techniques for large screen displays. The game setting with its drunken protagonist was well received by the players. This single player game uses the angle of the player's torso in relation to the ground to help Ed keep balance in a wobbling world. When facing obstacles, a short hop allows the player to make Ed leap over them. To trigger a selection in the menu we use the simple gesture of raising a hand to the mouth to drink. Ed's body pose reflects the player's pose, creating a very direct form of control. Results of our evaluation show that this form of mapping has proven to be very easy to learn and the short play sessions meet the requirements of a casual game in public environment.
...
% =============================================================================
The balance game \drunkened\ was developed with the goal of creating an application for public large screen displays, which could provide some sort of entertainment and raise awareness for the negative effects of drinking. 
\marginpar{
\fig{inGame2}{In game scene of Drunken Ed}
\fig{titleScreen}{Initial screen when no player is detected}
\fig{mainMenu}{Main menu to learn the controls and to select the difficulty}
\fig{paperPrototype}{The paper prototype to test the interaction}
}
...
% =============================================================================
\drunkened\ was created in a strongly participatory design process which entailed the creation of story boards (see \autoref{fig:storyboard1}) for use in semi structured interviews and paper prototyping for Wizard of Oz experiments (see \autoref{fig:paperPrototype}).
...
\marginpar{
\fig{storyboard1}{Storyboard of the interaction with the public display}
\fig{swingArms}{Ed spreading and swinging his arms as a warning}
\fig{fallDown1}{Ed while tumbling}
\fig{gameOver2}{Game over overlay after doing a highscore}
}
...
Creating games for public displays brings many new challenges in game design, ranging from the novel input forms, varyingThe game is still under development. Right now, placeholder graphics are replaced and new obstacle elements are added to enhance the gameplay.
